...
async function init() {
  ...
  try {
    ...
    showError(
      "No se pudo cargar la cotización. Verifica que el ID sea correcto.",
    );
  }
}
...
async function loadCotizacion(quoteId) {
  ...
  // Find the client
  const cliente = findClientBySlug(
    cotizacion.cliente.client_slug,
...
function renderContent(cotizacion, markdownContent) {
  ...
  // Convert marked's <pre><code class="language-mermaid"> to <div class="mermaid">
